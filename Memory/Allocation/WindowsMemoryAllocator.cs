...
public class WindowsMemoryAllocator : IMemoryAllocator
{
    ...
    {
        var allocationBase = WindowsPinvoke.VirtualAllocEx(processHandler.Process.Handle, address.HasValue ? (IntPtr)address : (IntPtr)null, new UIntPtr(sizeInBytes), AllocationTypeFlags.Commit | AllocationTypeFlags.Reserve, MemoryProtectionFlags.ExecuteReadWrite);
